  ...
  for size in xrange(1000, 11000, 1000):
    ...
    for j in range(0,2000):
      ...
      for i, bins in data_bins.iteritems():
        ...
        for ib in range(start, stop):
          gt_sample.append(bins[ib])
        gt_sample_total.extend(gt_sample)
      gt.append(func(gt_sample_total))
      if j%50 == 0 and j <= 600:
...
def main():
  ...
  #Compute Values on original data
  od = {}
  od_sample = {}
  dropped = {}
  random.shuffle(data)
  ...
  for k, v in fn.iteritems():
    od[v] = k(data)
    od_sample[v] = k(sample_data)
    print v, od[v], od_sample[v]
    #Also compute on data samples
  ...
  for K in range(5,101,5):
    ...
    for k,v in histogram.iteritems():
      ...
      for i in range(0,elem):
        dist.append(k)
    print "DROPPED", dropped[K], "out of", total
    dropped[K] = 1- float(dropped[K])/total
    random.shuffle(dist)
    sample_dist = []
    for i in range(0,50000):
      sample_dist.append(dist[i])
    for k,v in fn.iteritems():
      if v not in influence:
        influence[v] = {}
      if v not in influence_sample:
        influence_sample[v] = {}
      influence[v][K] = k(dist)
      influence_sample[v][K] = k(sample_dist)
      print K, v, influence[v][K], influence_sample[v][K]

  for k,v in od.iteritems():
    f = open('Influence-'+k, "w")
    f2 = open('SampleInfluence-'+k, "w")
    for K in range(5,101,5):
      f.write(str(K)+'\t'+str(abs(float(od[k]) - od_sample[k])/od[k]) + '\t' + str(abs(float(od[k]) - influence[k][K])/od[k]) + '\t' + str(abs(float(od[k]) - influence_sample[k][K])/od[k])+'\n')
      f2.write(str(K) + '\t' + str(abs(float(od_sample[k]) - influence_sample[k][K])/od_sample[k]) + '\n')
    f.close()
    f2.close()

  f = open("size-drop.txt", "w")
  for k,v in dropped.iteritems():
    f.write(str(k)+'\t'+str(v)+'\n')
  f.close()
